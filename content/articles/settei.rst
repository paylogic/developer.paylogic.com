...
:code:`settei` is a general purpose python settings library based on the concept of
...
:code:`settei` is a general purpose python settings library which uses entry
...
on the concept of entry points, in order to create a framework-agnostic library
...
settings from each other. However, we can specify the same environment names that
